...
fn render_lib_rs(spec: &ProcDeriveSpec) -> String {
    ...
    let body = spec.derive_fn_body();

    format!(
        r#"// GENERATED by tatara-rust-derive from a ProcDeriveSpec.
// Do not hand-edit; regenerate from the (defprocderive …) source.
...
mod tests {
    ...
    fn lib_rs_declares_derive_fn() {
        ...
        assert!(lib.contains("#[proc_macro_derive(StaticName)]"));
        assert!(lib.contains("pub fn derive_static_name"));
        assert!(lib.contains("parse_macro_input!(input as DeriveInput)"));
    }
